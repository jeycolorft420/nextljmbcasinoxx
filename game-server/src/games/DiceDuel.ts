...
export class DiceDuelGame {
    ...
    constructor(roomId: string, io: Server, prisma: PrismaClient, initialRoomData: any) {
        this.roomId = roomId;
        this.io = io;
        this.prisma = prisma;
        ...
        if (initialRoomData.entries) {
            initialRoomData.entries.forEach((e: any) => {
    ...
    addPlayerToState(p: Omit<Player, "connected"> & { connected?: boolean }) {
    ...
    async handleRoll(userId: string) {
        if (this.state.state !== "LOCKED") return;

        const player = this.state.players.find(p => p.id === userId);
        if (!player) return;

        if (this.state.rolls[userId]) return;
    ...
    async resolveRound(p1: Player, p2: Player) {
    ...
    async endGame(p1: Player, p2: Player) {
    ...
    async getEntryId(userId: string) {
    ...
    async tick() {
        if (this.state.state === "LOCKED") {
            ...
            if (now - this.state.roundStartedAt > 35000) {
                ...
                this.state.players.forEach(p => {
                    if (!this.state.rolls[p.id]) this.handleRoll(p.id);
                });
            }
        }
